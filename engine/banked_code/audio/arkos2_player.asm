;;
;; RAGE1 - Arkos2 AKG player — SHARED canonical copy (Phase AU4-4 of
;; doc/multiplatform-plan/audio.md).
;;
;; This is the single, canonical copy of the Arkos2 AKG player asm, used by
;; both the ZX (audio_zx_ay) and — eventually (AU5) — the CPC (audio_cpc_ay)
;; backends. It is pulled in by per-hardware wrapper .inc files:
;;   - engine/banked_code/128/arkos2-player_asm.inc        (sets _HARDWARE_SPECTRUM)
;;   - engine/banked_code/audio/.. CPC wrapper (AU5)        (sets _HARDWARE_CPC)
;;
...
;; load-bearing byte-identity guarantee of AU4-4). These lines emit no bytes.
;;
IFNDEF PLY_AKG_HARDWARE_SPECTRUM
  IFNDEF PLY_AKG_HARDWARE_CPC
    defc PLY_AKG_HARDWARE_SPECTRUM = 1
  ENDIF
ENDIF
...
;;
;; --- HARDWARE-SPECIFIC: PSG register output ------------------------------
;; The SPECTRUM branch below is BYTE-FOR-BYTE the pre-move ZX player: it pokes
;; the AY via the Spectrum register/data ports (49151 = 0xBFFD data,
;; 65533 = 0xFFFD register-select) using the `out (c)` 16-bit-port form. All
;; the PLY_AKG_PSGREG* / RETRIG / PSGREG13 self-modifying-code operand labels
;; that the init code patches live in this branch. The CPC PSG is driven via
;; the PPI (8255) ports — that branch is a Phase AU5 placeholder.
;;
IF PLY_AKG_HARDWARE_SPECTRUM
	ex af,af'
	ld de,49151
	ld bc,65533
	ld a,1
