...
void hero_reset_position(void) {
    ...
    animation_frame = current_assets.all_sprite_graphics[ h->num_graphic ].frame_data.frames[
        current_assets.all_sprite_graphics[ h->num_graphic ].sequence_data.sequences[ anim->current_sequence ].frame_numbers[ 0 ]
        ];

    // set initial position and move it there
    hero_set_position_x( h, current_assets.all_screens[ game_state.current_screen ].hero_data.startup_x );
    hero_set_position_y( h, current_assets.all_screens[ game_state.current_screen ].hero_data.startup_y );
...
void hero_set_position_x( struct hero_info_s *h, uint8_t x ) {
    h->position.x = x;
    h->position.xmax = h->position.x + current_assets.all_sprite_graphics[ h->num_graphic ].width - 1;
}

void hero_set_position_y( struct hero_info_s *h, uint8_t y ) {
    h->position.y = y;
    h->position.ymax = h->position.y + current_assets.all_sprite_graphics[ h->num_graphic ].height - 1;
}
...
uint8_t hero_can_move_in_direction( uint8_t direction ) {
    ...
    switch (direction ) {
        case MOVE_UP:
            ...
            c = PIXEL_TO_CELL_COORD( x + current_assets.all_sprite_graphics[ h->num_graphic ].width - 1 );
            ...
            break;
        case MOVE_DOWN:
            r = PIXEL_TO_CELL_COORD( y + current_assets.all_sprite_graphics[ h->num_graphic ].height - 1 + dy );
            c = PIXEL_TO_CELL_COORD( x + current_assets.all_sprite_graphics[ h->num_graphic ].width - 1 );
            for ( i = PIXEL_TO_CELL_COORD( x ) ; i <= c ; i++ )
                if ( TILE_TYPE_AT( r, i ) == TT_OBSTACLE )
                    return 0;
            ...
            break;
        case MOVE_LEFT:
            r = PIXEL_TO_CELL_COORD( y + current_assets.all_sprite_graphics[ h->num_graphic ].height - 1 );
        ...
        case MOVE_RIGHT:
            r = PIXEL_TO_CELL_COORD( y + current_assets.all_sprite_graphics[ h->num_graphic ].height - 1 );
            c = PIXEL_TO_CELL_COORD( x + current_assets.all_sprite_graphics[ h->num_graphic ].width - 1 + dx );
            for ( i = PIXEL_TO_CELL_COORD( y ) ; i <= r ; i++ )
                if ( TILE_TYPE_AT( i, c ) == TT_OBSTACLE )
                    return 0;
            return 1;
...
void hero_animate_and_move( void ) {
    ...
    switch ( controller & MOVE_ALL ) {
        ...
        case MOVE_DOWN:
            ...
            allowed = CELL_TO_PIXEL_COORD( GAME_AREA_BOTTOM + 1 ) - 1 - current_assets.all_sprite_graphics[ h->num_graphic ].height;
            if ( newy >= allowed )
                pos->y = allowed;
            else
        ...
        case MOVE_RIGHT:
            ...
            allowed = CELL_TO_PIXEL_COORD( GAME_AREA_RIGHT + 1 ) - 1 - current_assets.all_sprite_graphics[ h->num_graphic ].width;
            if ( newx >= allowed )
                pos->x = allowed;
            else
    ...
    animation_frame = current_assets.all_sprite_graphics[ h->num_graphic ].frame_data.frames[
        current_assets.all_sprite_graphics[ h->num_graphic ].sequence_data.sequences[ anim->current_sequence ].frame_numbers[ anim->current_frame ]
        ];

    // animate hero
    if ( ++anim->delay_counter == anim->delay ) {
        anim->delay_counter = 0;
        if ( ++anim->current_frame == current_assets.all_sprite_graphics[ h->num_graphic ].sequence_data.sequences[ anim->current_sequence ].num_elements ) {
            anim->current_frame = 0;
        }
    }
    ...
    if ( ( oldx != pos->x ) || ( oldy != pos->y ) ) {
        pos->xmax = pos->x + current_assets.all_sprite_graphics[ h->num_graphic ].width - 1;
        pos->ymax = pos->y + current_assets.all_sprite_graphics[ h->num_graphic ].height - 1;
    ...
    while ( i-- ) {
        ...
        while ( j-- ) {
            ...
            if ( TILE_TYPE_AT( r, c ) == TT_ITEM ) {
                item_loc = map_get_item_location_at_position( &current_assets.all_screens[ game_state.current_screen ], r, c );
                ...
                btile_remove( item_loc->row, item_loc->col, &current_assets.all_btiles[ all_items[ item ].btile_num ] );
...
void hero_update_lives_display(void) {
    ...
    while ( n-- ) {
        btile_draw( LIVES_AREA_TOP, col, &current_assets.all_btiles[ game_state.hero.lives_btile_num], TT_DECORATION, &lives_area );
        col += current_assets.all_btiles[ game_state.hero.lives_btile_num ].num_cols;
    }
}
...
void hero_init_sprites(void) {
    game_state.hero.sprite = hero_sprite = sprite_allocate(
        current_assets.all_sprite_graphics[ game_state.hero.num_graphic ].height >> 3,
        current_assets.all_sprite_graphics[ game_state.hero.num_graphic ].width >> 3
    );
}
