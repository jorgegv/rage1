...
void check_game_flags( void ) {
      ...
      if ( GET_LOOP_FLAG( F_LOOP_ENTER_SCREEN ) || GET_GAME_FLAG( F_GAME_START )) {
         ...
         map_draw_screen( &banked_assets->all_screens[ game_state.current_screen ] );
         enemy_reset_position_all( 
            banked_assets->all_screens[ game_state.current_screen ].enemy_data.num_enemies, 
            banked_assets->all_screens[ game_state.current_screen ].enemy_data.enemies
         );
         bullet_reset_all();
...
//      show_heartbeat();
   ...
   map_exit_screen( &banked_assets->all_screens[ game_state.current_screen ] );

   // move all moving things off-screen:
   hero_move_offscreen();
   enemy_move_offscreen_all( banked_assets->all_screens[ game_state.current_screen ].enemy_data.num_enemies,
      banked_assets->all_screens[ game_state.current_screen ].enemy_data.enemies );
