...
void check_flow_rules(void) {
    ...
    if ( banked_assets->all_screens[ game_state.current_screen ].flow_data.rule_tables.game_loop.num_rules )
        run_flow_rule_table( &banked_assets->all_screens[ game_state.current_screen ].flow_data.rule_tables.game_loop );
    ...
    if ( GET_LOOP_FLAG( F_LOOP_ENTER_SCREEN ) ) {
        ...
        if ( banked_assets->all_screens[ game_state.current_screen ].flow_data.rule_tables.enter_screen.num_rules )
            run_flow_rule_table( &banked_assets->all_screens[ game_state.current_screen ].flow_data.rule_tables.enter_screen );
    }
}
