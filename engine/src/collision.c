...
void collision_check_hero_with_sprites(void) {
    ...
    while ( i-- ) {
        ...
        if ( IS_ENEMY_ACTIVE( all_screen_asset_state_tables[ game_state.current_screen ][ s->state_index ].asset_state ) ) {
            ...
            while ( si-- ) {
                ...
                if ( IS_ENEMY_ACTIVE( all_screen_asset_state_tables[ game_state.current_screen ][ s->state_index ].asset_state ) ) {
                    if ( collision_check( &b->position, &s->position ) ) {
                        ...
                        RESET_ENEMY_FLAG( all_screen_asset_state_tables[ game_state.current_screen ][ s->state_index ].asset_state, F_ENEMY_ACTIVE );
