...
// gfx_jsp.h provide so the whole RAGE1 engine type-checks under +cpc.  At
// Phase G7 it is a STUB: macros expand to no-ops (do{}while(0)) or route
// through gfx_cpctel_* externs whose real bodies land in Phase G8 (real CPC
// backend wiring).  NO real CPC rendering happens here; the goal is C-level
// type-checking of engine/src/*.c against the stub backend (linkage may fail).
//
...
//--- Backend externs (STUB bodies in gfx_cpctel.c; real bodies in Phase G8) ---
// Declared, not necessarily linked — at G7 linkage may fail and that is OK.
