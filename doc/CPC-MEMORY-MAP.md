...
0x8000 ╞════════════════════════════════════════════════╡  page C (RAM2, always mapped)
       │ DSBUF - dataset decompress buffer (12 KB):     │
       │   0x8000 .. 0xAFFF (.. page_c_data_base)       │
0xB000 ├────────────────────────────────────────────────┤  page_c_data_base
       │ engine BSS (grows UP) 0xB000-0xBEFF            │
0xBF00 ├────────────────────────────────────────────────┤  REGISTER_SP-256 .. stack
       │ STACK (grows DOWN from SP) 0xBF00-0xBFFF       │
0xC000 ╞════════════════════════════════════════════════╡  page D (RAM3) / SP=0xC000
       │ SCREEN RAM (CRTC)                              │
0xFFFF └────────────────────────────────────────────────┘
```
...
| `0x8000-0xAFFF`  | **DSBUF** — dataset decompress buffer (12 KB).                     |
| `0xB000-0xBEFF`  | Engine BSS (relocated to page C; grows up). = `page_c_data_base`.  |
| `0xBF00-0xBFFF`  | Stack (grows down from `REGISTER_SP = 0xC000`).                    |
...
- **Only BSS is relocated to page C** (`page_c_data_base`). rodata/data stay
  contiguous with code in the single RUN"-loaded image — org-jumping initialised
  data makes z88dk emit a separate binary that the single load would not pick up.
- **DSBUF / BSS / stack share page C's 16 KB**, top-down: stack at the top
  (`REGISTER_SP = 0xC000`), BSS below it from `page_c_data_base`, DSBUF the rest
  down to `0x8000`. `page_c_data_base` is tunable per game.
