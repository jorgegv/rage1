...
### Possible enhancement: READ-WRITE pages (vs. READ-ONLY)
...
### Possible enhancement: compressed pages

In order to cram more data into the same space, page contents can be stored
compressed in the banks (ZX0), and decompressed on the fly when loaded into
the LOWMEM page frame. This way of working matches perfectly the memory
setup, since there is always a "decompression zone" ready which is the page
frame.

For this to work, a couple of changes are needed to our memory manager
design:

- The page data structure for the memory manager needs to have one more flag
  bit to indicate if the page is compressed

- The `page_in` function checks this flag and just copies or decompresses the
  source data into the LOWMEM page frame as needed

- The 2-bit field which indicates the page number inside a given bank (0-3)
  must be changed to a full 16 bit offset, which indicates the address
  offset inside the memory bank: since we are putting together several
  compressed pages in a 16K memory bank, we need to know the starting
  address for each of the compressed pages in order to decompress.

- Since the pages and offsets are then non-deterministic and depend on the
  size of the compressed pages, the memory map must be generated by the same
  tool that generates the game data (i.e.  DATAGEN and FLOWGEN).

This schema only works for READ-ONLY pages, which are just decompressed and
used.

READ-WRITE pages would need to be compressed when paged out (which would
probably incur a heavy performance hit), and also the newly compressed pages
would not match the original compressed size if their contents have changed,
which would complicate very much the memory manager design.
