...
### Patching automatically generated SCREEN definitions

The MAPGEN tool provided with RAGE1 can be used to automate most of the map
screen definition workflow (see [MAPGEN](MAPGEN.md) document for details on
how to use it).

This tool can generate the SCREEN definitions in the regular directory
(`game_data/map`) and for this reason, a patching mechanism has been
provided which can be used to add some definitions to an already existing screen.
This mechanism allows to have separate SCREEN definitions (which may be
automatically generated by MAPGEN) and custom user modifications in
different files.

The SCREEN configuration patches are also provided in regular GDATA files,
stored in directory `game_data/patches/map`. The syntax for a patch file is
the following:

```
PATCH_SCREEN NAME=AsylumGrass
	ENEMY           NAME=Nurse1       SPRITE=Nurse (...etc.)
	(...)
END_SCREEN
```

For the patch to be applied, a new `PATCH_SCREEN` directive has to be used,
which requires a `NAME=<some_name>` parameter.  Besides this, the elements
