...
for i in $DATASETS; do
	...
	printf "Dataset %2d: %5db - Arena: %4db - BTtab: %3d(%4db) BTptrs: %3d(%4db) - BTpos: %3d(%4db) - Screens: %3d(%4db) - Rest: %4db\n" \
		"$i" "$DS_SIZE" "$ARENA_SIZE" "$TILETAB_COUNT" "$TILETAB_SIZE" "$TILEPTR_COUNT" "$TILEPTR_SIZE" "$BTILEPOS_COUNT" "$BTILEPOS_SIZE" "$SCREEN_COUNT" "$SCREEN_SIZE" "$REST"
done
