...
## reached via their RAGE::Datagen::Context aliases (@main::all_sprites,
## %main::dataset_dependency, $main::c_dataset_lines — the per-dataset C emit
...
## main::asset_backend().  Behaviour (and emitted bytes) is unchanged.
##
################################################################################
...
sub generate_sprite {
    ...
    push @{ $main::c_dataset_lines->{ $dataset } }, sprintf( "// Sprite '%s'\n// Pixel and mask data ordered by column (%s format)\n\n",
    ...
    my ( $data_bytes, $frame_offsets ) = main::asset_backend()->sprite_frame_data( $sprite );
    ...
    push @{ $main::c_dataset_lines->{ $dataset } }, sprintf( "uint8_t sprite_%s_data[] = {\n%s\n};\n",
        $sprite->{'name'},
        join( ",\n", map { join( ", ", map { sprintf( "0x%02x", $_ ) } @{$_} ) } @groups_of_2m ) );

    # output list of pointers to frames (offsets owned by the backend)
    push @{ $main::c_dataset_lines->{ $dataset } }, sprintf( "uint8_t *sprite_%s_frames[] = {\n%s\n};\n",
        $sprite_name,
        join( ",\n",
    ...
    if ( scalar( @{ $sprite->{'sequences'} } ) ) {
        push @{ $main::c_dataset_lines->{ $dataset } }, join( "", map {
            sprintf( "uint8_t sprite_%s_sequence_%s[%d] = { %s };\n",
                $sprite_name, $_->{'name'}, scalar( @{ $_->{'frame_list'} } ), join( ',', @{ $_->{'frame_list'} } ) );
        } @{ $sprite->{'sequences'} } );
        push @{ $main::c_dataset_lines->{ $dataset } }, sprintf( "struct animation_sequence_s sprite_%s_sequences[%d] = {\n\t",
    ...
    push @{ $main::c_dataset_lines->{ $dataset } }, sprintf( "// End of Sprite '%s'\n\n", $sprite_name );
}
...
sub generate_sprites {
    ...
    my @dataset_sprites = map { $main::all_sprites[ $_ ] } @{ $main::dataset_dependency{ $dataset }{'sprites'} };
    return if not scalar( @dataset_sprites );

    push @{ $main::c_dataset_lines->{ $dataset } }, <<EOF_SPRITES
...
;
    ...
    push @{ $main::c_dataset_lines->{ $dataset } }, "// Dataset sprite graphics table\n";
    push @{ $main::c_dataset_lines->{ $dataset } }, "struct sprite_graphic_data_s all_sprite_graphics[ $num_sprites ] = {\n\t";
    push @{ $main::c_dataset_lines->{ $dataset } }, join( ",\n\n\t", map {
        my $sprite = $_;
