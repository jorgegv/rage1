...
## only edits are the mechanical scaffold bindings: the shared globals are
## reached via their RAGE::Datagen::Context aliases ($main::game_config,
## @main::all_screens, @main::all_sprites, %main::sprite_name_to_index,
## %main::screen_name_to_index, $main::hero, $main::max_flow_var_id,
## %main::conditional_build_features, $main::cfg, $main::build_dir,
## @main::valid_game_functions and the emit accumulators @main::c_game_data_lines,
## @main::h_game_data_lines and @main::h_build_features_lines).
...
sub generate_game_functions {
    push @main::h_game_data_lines, "// game config\n";

    # generate extern declarations, only for functions in 'home' codeset
    push @main::h_game_data_lines, join( "\n",
        map {
            sprintf( "void %s( void );", $main::game_config->{'game_functions'}{ $_ }{'name'} )
        } grep {
            ( $main::game_config->{'zx_target'} eq '48' ) or
            ( $main::game_config->{'game_functions'}{ $_ }{'codeset'} eq 'home' )
        } grep {
            $_ ne 'custom'
        } sort keys %{ $main::game_config->{'game_functions'} } );
    ...
    push @main::h_game_data_lines, join( "\n",
        map {
            sprintf( "#define run_game_function_%-30s %s",
                lc( $_ ) . '()',
                ( defined( $main::game_config->{'game_functions'}{ $_ } ) ?
                  $main::game_config->{'game_functions'}{ $_ }{'codeset_function_call_macro'} :
                  '' ),
            )
        } grep {
            $_ ne 'custom'
        } sort @main::valid_game_functions
    );
...
sub generate_single_game_area {
    my $area = shift;
    return if not defined ( $main::game_config->{ $area } );
    ...
    push @main::h_game_data_lines, "\n" . join( "\n", map {
            "// " .uc( $_ ). " definitions\n" .
            sprintf( "#define %s_TOP	%d\n", uc( $_ ), $main::game_config->{ $_ }{'top'} ) .
            sprintf( "#define %s_LEFT	%d\n", uc( $_ ), $main::game_config->{ $_ }{'left'} ) .
            sprintf( "#define %s_BOTTOM	%d\n", uc( $_ ), $main::game_config->{ $_ }{'bottom'} ) .
            sprintf( "#define %s_RIGHT	%d\n", uc( $_ ), $main::game_config->{ $_ }{'right'} ) .
...
sub generate_game_areas {
    ...
    push @main::c_game_data_lines, "// screen areas\n";
...
sub generate_h_header {
    push @main::h_game_data_lines, <<GAME_DATA_H_1
#ifndef _GAME_DATA_H
#define _GAME_DATA_H
...
sub generate_h_ending {
    push @main::h_game_data_lines, <<GAME_DATA_H_4
#endif // _GAME_DATA_H
GAME_DATA_H_4
;
}
...
sub generate_game_config {
    ...
    push @main::h_game_data_lines, sprintf( "#define MAP_NUM_SCREENS\t%d\n", scalar( @main::all_screens ) );
    push @main::h_game_data_lines, sprintf( "#define MAP_INITIAL_SCREEN\t%d\n", $main::screen_name_to_index{ $main::game_config->{'screen'}{'initial'} } );
    push @main::h_game_data_lines, sprintf( "#define DEFAULT_BG_ATTR ( %s )\n", $main::game_config->{'default_bg_attr'} );

    push @main::h_game_data_lines, "\n// sound effect constants\n";
    foreach my $effect ( keys %{$main::game_config->{'sounds'}} ) {
        push @main::h_game_data_lines, sprintf( "#define SOUND_%s %s\n", uc( $effect ), $main::game_config->{'sounds'}{ $effect } );
    }
    ...
    foreach my $screen ( @main::all_screens ) {
        ...
        foreach my $sprite ( map { $main::all_sprites[ $main::sprite_name_to_index{ $_->{'sprite'} } ] } @{ $screen->{'enemies'} } ) {
            $screen_sprites++;
    ...
    $max_sprites++;
    my $hs = $main::all_sprites[ $main::sprite_name_to_index{ $main::hero->{'sprite'} } ];
    $max_spritechars += ( $hs->{'rows'} + 1 ) * ( $hs->{'cols'} + 1 );

    # add the bullet sprites - the N bullets
    if ( defined( $main::hero->{'bullet'} ) ) {
        $max_sprites += $main::hero->{'bullet'}{'max_bullets'};
        my $bs = $main::all_sprites[ $main::sprite_name_to_index{ $main::hero->{'bullet'}{'sprite'} } ];
        $max_spritechars += $main::hero->{'bullet'}{'max_bullets'} * ( $bs->{'rows'} + 1 ) * ( $bs->{'cols'} + 1 );
    }

    # JSP pool-sizing constants (only emitted when gfx backend is JSP)
    if ( get_gfx_backend() eq 'jsp' ) {
        my $max_sprite_rows = ( sort { $b <=> $a } map { $_->{'rows'} } @main::all_sprites )[0];
        my $max_sprite_cols = ( sort { $b <=> $a } map { $_->{'cols'} } @main::all_sprites )[0];
        push @main::h_game_data_lines, <<EOF_JSP_POOL
...
;
    ...
    my $max_dataset_size = (
        ( $main::cfg->{ $int_key }{'base_code_address'} =~ /^0x/ ?
            hex( $main::cfg->{ $int_key }{'base_code_address'} ) :
            $main::cfg->{ $int_key }{'base_code_address'} )
         - 0x5B00 ) - $max_heap_usage;

    push @main::h_game_data_lines, <<EOF_BLDCFG1
...
;
    ...
    if ( defined( $main::game_config->{'custom_charset'} ) ) {
        my ( $char_min, $char_max ) = ( 32, 127 );	# defaults: all ZX ASCII range
        if ( $main::game_config->{'custom_charset'}{'range'} ) {
            $main::game_config->{'custom_charset'}{'range'} =~ m/^(\d+)\-(\d+)$/;
            ( $char_min, $char_max ) = ( $1, $2 );
        }
        push @main::h_game_data_lines, "\n// custom charset minimum and maximum characters\n";
        push @main::h_game_data_lines, sprintf( "#define CUSTOM_CHARSET_MIN_CHAR %d\n", $char_min );
        push @main::h_game_data_lines, sprintf( "#define CUSTOM_CHARSET_MAX_CHAR %d\n", $char_max );

        # ...and CUSTOM_CHARSET data to game_data.c
        open( CHAR_DATA, "$main::build_dir/$main::game_config->{'custom_charset'}{'file'}" ) or
            die "CUSTOM_CHARSET: could not open $main::game_config->{'custom_charset'}{'file'} for reading\n";
        binmode CHAR_DATA;
        my $data;
        my $data_offset = ( $char_min - 32 ) * 8;
        my $data_size = ( $char_max - $char_min + 1 ) * 8;
        if ( read( CHAR_DATA, $data, $data_size, $data_offset ) != $data_size ) {
            die "CUSTOM_CHARSET: error reading $data_size bytes from $main::game_config->{'custom_charset'}{'file'}, offset $data_offset\n";
        }
        close CHAR_DATA;
        push @main::c_game_data_lines, "\n// custom charset binary data\n";
        push @main::c_game_data_lines, sprintf( "// first char: %d ('%c'), last char: %d ('%c')\n", $char_min, $char_min, $char_max, $char_max );
        push @main::c_game_data_lines, "uint8_t custom_charset[ ( CUSTOM_CHARSET_MAX_CHAR - CUSTOM_CHARSET_MIN_CHAR + 1 ) * 8 ] = {\n";
        push @main::c_game_data_lines, "\t" . join( ", ", map { sprintf "0x%02x", $_ } unpack('C*', $data ) ) . "\n";
        push @main::c_game_data_lines, "};\n\n";
    }

    # add gamearea default attribute if monochrome mode is used
    if ( defined( $main::game_config->{'color'}{'gamearea_attr'} ) ) {
        push @main::h_game_data_lines, "\n// gamearea default attr for monochrome mode\n";
        push @main::h_game_data_lines, sprintf( "#define GAMEAREA_COLOR_MONO_ATTR (%s)\n\n", $main::game_config->{'color'}{'gamearea_attr'} );
    }

    # add custom_state_data config
    if ( defined( $main::game_config->{'custom_state_data'} ) ) {
        push @main::h_game_data_lines, "\n// custom state data size\n";
        push @main::h_game_data_lines, sprintf( "#define CUSTOM_STATE_DATA_SIZE %d\n\n", $main::game_config->{'custom_state_data'}{'size'} );
    }
...
sub generate_misc_data {
    ...
    push @main::h_game_data_lines, sprintf( "#define\tGAME_NUM_TOTAL_ENEMIES\t%d\n\n", $count );

    # number of flow vars
    if ( defined( $main::max_flow_var_id ) ) {
        push @main::h_game_data_lines, "\n// Total number of flow vars in the game\n";
        push @main::h_game_data_lines, sprintf( "#define\tGAME_NUM_FLOW_VARS\t%d\n\n", ( $main::max_flow_var_id + 1 ) );
    }
}
...
sub generate_conditional_build_features {
    ...
    push @main::h_build_features_lines, <<EOF_FEATURES1
...
;
    ...
    foreach my $f ( sort keys %main::conditional_build_features ) {
        push @main::h_build_features_lines, sprintf( "#define BUILD_FEATURE_%s\n", uc($f) );
    }

    push @main::h_build_features_lines, <<EOF_FEATURES2
...
sub generate_configuration_values {
    ...
    my $int_key = 'interrupts_128';
    push @main::h_game_data_lines, "// Interrupt configuration\n";
    foreach my $k ( qw( iv_table_addr isr_vector_byte base_code_address ) ) {
        my $value = $main::cfg->{ $int_key }{ $k };
        if ( $value =~ /^0x/ ) {
            $value = hex( $value );
        }
        push @main::h_game_data_lines, sprintf( "#define RAGE1_CONFIG_INT128_%-29s 0x%x\n", uc($k), $value );
        if ( $k eq 'isr_vector_byte' ) {
            push @main::h_game_data_lines, sprintf( "#define RAGE1_CONFIG_INT128_ISR_ADDRESS                   0x%02x%02x\n",
                $value, $value
            );
        }
    }
