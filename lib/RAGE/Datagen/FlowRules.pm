...
## tools/datagen.pl; the only edits are the mechanical scaffold bindings: the
## shared globals are reached via their RAGE::Datagen::Context aliases
## (@main::all_rules, @main::all_screens, @main::game_events_rule_table,
## %main::screen_name_to_index, %main::dataset_dependency,
## %main::check_custom_function_id, @main::check_custom_functions,
## %main::action_custom_function_id, @main::action_custom_functions,
## $main::max_flow_var_id, $main::game_config, $main::syntax and the per-dataset
## C emit accumulator $main::c_dataset_lines).  add_build_feature is imported
...
sub find_existing_rule_index {
    my $rule = shift;
    foreach my $i ( 0 .. ( scalar( @main::all_rules ) - 1 ) ) {
        return $i if Compare( $rule, $main::all_rules[ $i ] );
    }
    return undef;
}
...
sub validate_and_compile_rule {
    ...
    if ( $screen ne '__EVENTS__' ) {
        exists( $main::screen_name_to_index{ $screen } ) or
    ...
    if ( $rule->{'screen'} ne '__EVENTS__' ) {
        ...
        my $when = $rule->{'when'};
        grep { $when eq $_ } @{ $main::syntax->{'valid_whens'} } or
            die "WHEN must be one of ".join( ", ", map { uc } @{ $main::syntax->{'valid_whens'} } )."\n";
    }
...
#        die "At least one CHECK clause must be specified\n";
    ...
    foreach my $chk ( @{ $rule->{'check'} } ) {
        ...
        if ( $check =~ /^HERO_OVER_HOTZONE$/ ) {
            $check_data = $main::all_screens[ $main::screen_name_to_index{ $rule->{'screen'} } ]{'hotzone_name_to_index'}{ $check_data };
        }
        ...
        if ( $check =~ /^CALL_CUSTOM_FUNCTION/ ) {
            ...
            my $index;
            if ( defined( $main::check_custom_function_id{ $vars->{'name'} } ) ) {
                $index = $main::check_custom_function_id{ $vars->{'name'} };
            } else {
                $index = scalar( @main::check_custom_functions );
                $main::check_custom_function_id{ $vars->{'name'} } = $index;
                push @main::check_custom_functions, {
                    index		=> $index,
                    function		=> $vars->{'name'},
        ...
        if ( $check =~ /^FLOW_VAR/ ) {
            ...
            if ( not defined( $main::max_flow_var_id ) or ( $vars->{'var_id'} > $main::max_flow_var_id ) ) {
                $main::max_flow_var_id = $vars->{'var_id'};
            }
    ...
    foreach my $do ( @{ $rule->{'do'} } ) {
        ...
        if ( $action =~ /^(ENABLE|DISABLE)_HOTZONE$/ ) {
            $action_data = $main::all_screens[ $main::screen_name_to_index{ $rule->{'screen'} } ]{'hotzone_name_to_index'}{ $action_data };
        }
        ...
        if ( $action =~ /^WARP_TO_SCREEN$/ ) {
            ...
            $action_data = sprintf( "{ .num_screen = %d, .hero_x = %d, .hero_y = %d, .flags = %s }",
                $main::screen_name_to_index{ $vars->{'dest_screen'} },
        ...
        if ( $action =~ /^(ENABLE|DISABLE)_BTILE$/ ) {
            $action_data = $main::all_screens[ $main::screen_name_to_index{ $rule->{'screen'} } ]{'btile_name_to_index'}{ $action_data };
        }

        # enemy filtering
        if ( $action =~ /^(ENABLE|DISABLE)_ENEMY$/ ) {
            $action_data = $main::all_screens[ $main::screen_name_to_index{ $rule->{'screen'} } ]{'enemy_name_to_index'}{ $action_data };
        }
        ...
        if ( $action =~ /^(SET|RESET)_SCREEN_FLAG$/ ) {
            ...
            $action_data = sprintf( "{ .num_screen = %d, .flag = %s }",
                $main::screen_name_to_index{ $vars->{'screen'} },
                ( $vars->{'flag'} || 0 ),
            );
        }
        ...
        if ( $action =~ /^CALL_CUSTOM_FUNCTION/ ) {
            ...
            defined( $vars->{'name'} ) or
                die "CALL_CUSTOM_FUNCTION: NAME parameter is mandatory\n";
            my $index;
            if ( defined( $main::action_custom_function_id{ $vars->{'name'} } ) ) {
                $index = $main::action_custom_function_id{ $vars->{'name'} };
            } else {
                $index = scalar( @main::action_custom_functions );
                $main::action_custom_function_id{ $vars->{'name'} } = $index;
                push @main::action_custom_functions, {
                    index		=> $index,
                    function		=> $vars->{'name'},
                    uses_param		=> ( defined( $vars->{'param'} ) ? 1 : 0 ),
                };
        ...
        if ( $action =~ /^TRACKER_SELECT_SONG/ ) {
            ...
            if ( defined( $main::game_config->{'tracker'}{'song_index'}{ $action_data } ) ) {
                $action_data = $main::game_config->{'tracker'}{'song_index'}{ $action_data };
            }
        }
...
sub generate_flow_rules {
    ...
    my @dataset_rules = map { $main::all_rules[ $_ ] } @{ $main::dataset_dependency{ $dataset }{'rules'} };
    return if not scalar( @dataset_rules );

    # file header comments
    push @{ $main::c_dataset_lines->{ $dataset } }, <<FLOW_DATA_C_1
...
;
    ...
    push @{ $main::c_dataset_lines->{ $dataset } }, sprintf( "// check tables for all dataset rules\n" );
    foreach my $i ( 0 .. scalar( @dataset_rules )-1 ) {
        push @{ $main::c_dataset_lines->{ $dataset } }, generate_rule_checks( $dataset_rules[ $i ], $i );
    }
    push @{ $main::c_dataset_lines->{ $dataset } }, sprintf( "// action tables for all dataset rules\n" );
    foreach my $i ( 0 .. scalar( @dataset_rules )-1 ) {
        push @{ $main::c_dataset_lines->{ $dataset } }, generate_rule_actions( $dataset_rules[ $i ], $i );
    }

    # output dataset rule table
    if ( scalar( @dataset_rules ) ) {
        push @{ $main::c_dataset_lines->{ $dataset } }, sprintf(  "// Dataset %s rule table\n\n#define FLOW_NUM_RULES\t%d\n",
            $dataset, scalar( @dataset_rules ) );
        push @{ $main::c_dataset_lines->{ $dataset } }, "struct flow_rule_s all_flow_rules[ FLOW_NUM_RULES ] = {\n";
        foreach my $i ( 0 .. scalar( @dataset_rules )-1 ) {
            push @{ $main::c_dataset_lines->{ $dataset } }, "\t{";
            push @{ $main::c_dataset_lines->{ $dataset } }, sprintf( " .num_checks = %d, .checks = %s,",
                scalar( @{ $dataset_rules[ $i ]{'check'} } ),
                ( scalar( @{ $dataset_rules[ $i ]{'check'} } ) ? sprintf( "&flow_rule_checks_%05d[0]", $i ) : 'NULL' )
            );
            push @{ $main::c_dataset_lines->{ $dataset } }, sprintf( " .num_actions = %d, .actions = &flow_rule_actions_%05d[0],",
...
sub generate_game_events_rule_table {
    ...
    my $rule_global_to_dataset_index = $main::dataset_dependency{ 'home' }{'rule_global_to_dataset_index'};

    # flow rules
    push @{ $main::c_dataset_lines->{ 'home' } }, sprintf( "// Game Events rule table\n" );
    my $num_rules = scalar( @main::game_events_rule_table );
    if ( $num_rules ) {
        push @{ $main::c_dataset_lines->{ 'home' } }, sprintf( "struct flow_rule_s *game_events_rule_table_rules[ %d ] = {\n\t",
            $num_rules );
        push @{ $main::c_dataset_lines->{ 'home' } }, join( ",\n\t",
            map {
                sprintf( "&all_flow_rules[ %d ]", $rule_global_to_dataset_index->{ $_ } )
            } @main::game_events_rule_table
        );
        push @{ $main::c_dataset_lines->{ 'home' } }, "\n};\n";
        push @{ $main::c_dataset_lines->{ 'home' } },
        ...
        push @{ $main::c_dataset_lines->{ 'home' } }, sprintf( "// Game Events rule table has no rules defined\n" );
        push @{ $main::c_dataset_lines->{ 'home' } },
