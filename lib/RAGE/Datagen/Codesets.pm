...
## in as the first positional arg of each sub and reached as $ctx->{game_config},
## $ctx->{c_game_data_lines}, $ctx->{h_game_data_lines}, $ctx->{c_banked_data_128_lines},
## $ctx->{c_tracker_cpc_lines}, $ctx->{c_dataset_lines}, $ctx->{asm_dataset_lines},
## $ctx->{c_codeset_lines}, $ctx->{asm_codeset_lines}, $ctx->{dataset_dependency},
## $ctx->{codeset_functions_by_codeset}, $ctx->{all_codeset_functions},
## $ctx->{codeset_valid_banks}, $ctx->{check_custom_functions},
## $ctx->{action_custom_functions}, $ctx->{dataset_base_address},
## $ctx->{codeset_base_address} and $ctx->{build_dir}.
##
...
sub generate_c_home_header {
    my $ctx = shift;
    push @{ $ctx->{c_game_data_lines} }, <<EOF_HEADER
...
sub generate_c_banked_header {
    ...
    my $num_btiles	= scalar( @{ $ctx->{dataset_dependency}{ $dataset }{'btiles'} } );
    my $num_sprites	= scalar( @{ $ctx->{dataset_dependency}{ $dataset }{'sprites'} } );
    my $num_flow_rules	= scalar( @{ $ctx->{dataset_dependency}{ $dataset }{'rules'} } );
    my $num_screens	= scalar( @{ $ctx->{dataset_dependency}{ $dataset }{'screens'} } );
    ...
    if ( $dataset =~ /^\d+$/ ) {
        push @{ $ctx->{c_dataset_lines}->{ $dataset } }, <<EOF_HEADER
///////////////////////////////////////////////////////////////////////////
//
...
;
        ...
        push @{ $ctx->{asm_dataset_lines}->{ $dataset } }, <<EOF_HEADER
        org	$ctx->{dataset_base_address}
EOF_HEADER
;
    }

    push @{ $ctx->{asm_dataset_lines}->{ $dataset } }, <<EOF_HEADER2
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
sub generate_global_codeset_data {
    my $ctx = shift;
    push @{ $ctx->{h_game_data_lines} }, <<EOF_CODESET_1
...
EOF_CODESET_1
;
    push @{ $ctx->{h_game_data_lines} }, sprintf( "#define	NUM_CODESETS	%d\n\n", scalar( grep { "$_" ne 'home' } keys %{ $ctx->{codeset_functions_by_codeset} } ) );
    push @{ $ctx->{c_game_data_lines} }, <<EOF_CODESET_3
...
;
    ...
    my @non_home_codeset_functions = grep { $_->{'codeset'} ne 'home' } @{ $ctx->{all_codeset_functions} };

    if ( scalar( @non_home_codeset_functions ) and ( $ctx->{game_config}->{'zx_target'} ne '48' ) ) {
        push @{ $ctx->{c_game_data_lines} }, "// global codeset functions table\n";
        push @{ $ctx->{h_game_data_lines} }, "// global indexes of codeset functions\n";

        push @{ $ctx->{c_game_data_lines} }, sprintf( "struct codeset_function_info_s all_codeset_functions[ %d ] = { \n",
            scalar( @non_home_codeset_functions )
        );
        my $index = 0;
        foreach my $function ( @non_home_codeset_functions ) {
            push @{ $ctx->{c_game_data_lines} },  sprintf( "\t{ .bank_num = %d, .local_function_num = %d },\n",
                $ctx->{codeset_valid_banks}[ $function->{'codeset'} ],
                $function->{'local_index'},
            );
            push @{ $ctx->{h_game_data_lines} }, sprintf( "#define CODESET_FUNCTION_%s	(%d)\n",
                uc( $function->{'name'} ),
                $index,
            );
            $index++;
        ...
        push @{ $ctx->{c_game_data_lines} }, "// No codesets defined\n";
        push @{ $ctx->{h_game_data_lines} }, "// No codesets defined\n";
    }
    ...
    push @{ $ctx->{h_game_data_lines} }, "// codeset function call macros for each function\n";
    foreach my $function ( @{ $ctx->{all_codeset_functions} } ) {
        my $macro;
        if ( ( $ctx->{game_config}->{'zx_target'} eq '48' ) or ( $function->{'codeset'} eq 'home' ) ) {
            # macros for 48K mode
            push @{ $ctx->{h_game_data_lines} }, sprintf(
    ...
    push @{ $ctx->{h_game_data_lines} }, <<EOF_CODESET_2
...
;
    ...
    push @{ $ctx->{c_game_data_lines} }, <<EOF_CODESET_4
...
sub all_codesets_except_home {
    ...
    my @function_codesets = grep { "$_" ne 'home' } keys %{ $ctx->{codeset_functions_by_codeset} };

    # codesets used by binary items
    my @blob_codesets = grep { "$_" ne 'home' } map { $_->{'codeset'} } @{ $ctx->{game_config}->{'binary_data'} };
...
sub generate_codeset_headers {
    ...
    foreach my $codeset ( all_codesets_except_home( $ctx ) ) {
        ...
        push @{ $ctx->{asm_codeset_lines}->{ $codeset } }, <<EOF_CODESET_LINES_MAIN_ASM
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
...
;
        ...
        push @{ $ctx->{c_codeset_lines}->{ $codeset } }, <<EOF_CODESET_LINES_MAIN
#include <stdlib.h>
...
sub generate_codeset_functions {
    ...
    foreach my $codeset ( all_codesets_except_home( $ctx ) ) {
        next if not defined( $ctx->{codeset_functions_by_codeset}{ $codeset } );
        my $num_codeset_functions = scalar( @{ $ctx->{codeset_functions_by_codeset}{ $codeset } } );
        if ( $num_codeset_functions ) {
            # add extern codeset function declarations
            push @{ $ctx->{c_codeset_lines}->{ $codeset } }, "// codeset functions table\n";
            foreach my $function ( @{ $ctx->{codeset_functions_by_codeset}{ $codeset } } ) {
                push @{ $ctx->{c_codeset_lines}->{ $codeset } }, sprintf( "extern void %s( void );\n", $function->{'name'} );
            }

            # add the codeset function table
            push @{ $ctx->{asm_codeset_lines}->{ $codeset } }, "	db	$num_codeset_functions			;; .num_functions\n";
            push @{ $ctx->{asm_codeset_lines}->{ $codeset } }, "	dw	_codeset_functions	;; .functions\n";
            push @{ $ctx->{c_codeset_lines}->{ $codeset } }, "codeset_function_t codeset_functions[ $num_codeset_functions ] = {\n";
            foreach my $function ( @{ $ctx->{codeset_functions_by_codeset}{ $codeset } } ) {
                push @{ $ctx->{c_codeset_lines}->{ $codeset } }, sprintf( "\t&%s,\n", $function->{'name'} );
            }
            push @{ $ctx->{c_codeset_lines}->{ $codeset } }, "};\n\n";
        }
    }
}
...
sub generate_custom_function_tables {
    ...
    if ( scalar( @{ $ctx->{check_custom_functions} } ) ) {

        push @{ $ctx->{h_game_data_lines} }, "// Check custom functions table\n";
        push @{ $ctx->{h_game_data_lines} }, "extern check_custom_function_t check_custom_functions[];\n\n";
        push @{ $ctx->{h_game_data_lines} }, "// Check custom function prototypes\n";

        push @{ $ctx->{c_game_data_lines} }, "// Check custom functions table\n";
        push @{ $ctx->{c_game_data_lines} }, sprintf( "check_custom_function_t check_custom_functions[ %d ] = {\n",
            scalar( @{ $ctx->{check_custom_functions} } )
        );
        foreach my $f ( @{ $ctx->{check_custom_functions} } ) {
            # all check functions must accept a single 1-byte param, even if they ignore it
            push @{ $ctx->{h_game_data_lines} }, sprintf( "uint8_t %s( %s );\n", $f->{'function'}, 'uint8_t param' );
            push @{ $ctx->{c_game_data_lines} }, sprintf( "\t%s,\n", $f->{'function'} );
        }
        push @{ $ctx->{h_game_data_lines} }, "\n";
        push @{ $ctx->{c_game_data_lines} }, "};\n\n";
    }
    # generate 'custom' type action function prototypes ('home' codeset)
    if ( scalar( @{ $ctx->{action_custom_functions} } ) ) {

        push @{ $ctx->{h_game_data_lines} }, "// Action custom functions table\n";
        push @{ $ctx->{h_game_data_lines} }, "extern action_custom_function_t action_custom_functions[];\n\n";
        push @{ $ctx->{h_game_data_lines} }, "// Action custom function prototypes\n";

        push @{ $ctx->{c_game_data_lines} }, "// Action custom function table\n";
        push @{ $ctx->{c_game_data_lines} }, sprintf( "action_custom_function_t action_custom_functions[ %d ] = {\n",
            scalar( @{ $ctx->{action_custom_functions} } )
        );
        foreach my $f ( @{ $ctx->{action_custom_functions} } ) {
            push @{ $ctx->{h_game_data_lines} }, sprintf( "void %s( %s );\n", $f->{'function'}, ( $f->{'uses_param'} ? 'uint8_t param' : 'void' ) );
            push @{ $ctx->{c_game_data_lines} }, sprintf( "\t%s,\n", $f->{'function'} );
        }
        push @{ $ctx->{h_game_data_lines} }, "\n";
        push @{ $ctx->{c_game_data_lines} }, "};\n\n";
    ...
    push @{ $ctx->{h_game_data_lines} }, "// Crumb actions functions table\n";
    foreach my $function ( grep { lc( $_->{'type'} ) eq 'crumb_action' } @{ $ctx->{codeset_functions_by_codeset}{'home'} } ) {
        push @{ $ctx->{h_game_data_lines} }, sprintf( "extern void %s(  struct crumb_info_s *c );\n", $function->{'name'} );
    }
}
...
sub generate_binary_data_items {
    ...
    return if ( not defined( $ctx->{game_config}->{'binary_data'} ) or not scalar( @{ $ctx->{game_config}->{'binary_data'} } ) );

    # general data
    push @{ $ctx->{h_game_data_lines} }, "// extern declarations for binary data items\n";
    push @{ $ctx->{c_game_data_lines} }, "//////////////////////////////////////////\n";
    push @{ $ctx->{c_game_data_lines} }, "// BINARY DATA ITEMS\n";
    push @{ $ctx->{c_game_data_lines} }, "//////////////////////////////////////////\n\n";

    # process each of the binary blobs and generate its code
    foreach my $item ( @{ $ctx->{game_config}->{'binary_data'} } ) {
        ...
        my @bytes;
        if ( $item->{'compress'} ) {
            @bytes = main::file_to_compressed_bytes( "$ctx->{build_dir}/$item->{'file'}", $item->{'offset'}, $item->{'size'} );
        } else {
            @bytes = main::file_to_bytes( "$ctx->{build_dir}/$item->{'file'}", $item->{'offset'}, $item->{'size'} );
        }
        ...
        if ( $ctx->{game_config}->{'zx_target'} eq '48' ) {
            # logic for 48K mode - easy, all into game_data.c and .h. CODESET setting is ignored
            push @{ $ctx->{h_game_data_lines} }, @h_lines;
            push @{ $ctx->{c_game_data_lines} }, @c_lines;
        } else {
            ...
            if ( $codeset eq 'home' ) {
                ...
                push @{ $ctx->{c_codeset_lines}->{ $codeset } }, @c_lines;
            }
        }
    }
    ...
    push @{ $ctx->{h_game_data_lines} }, "\n";
    push @{ $ctx->{c_game_data_lines} }, "//////////////////////////////////////////\n";
    push @{ $ctx->{c_game_data_lines} }, "// END OF BINARY DATA ITEMS\n";
    push @{ $ctx->{c_game_data_lines} }, "//////////////////////////////////////////\n";
...
sub generate_c_banked_data_128_header {
    my $ctx = shift;
    push @{ $ctx->{c_banked_data_128_lines} },<<EOF_BANKED_128_HEADING
#include <stdint.h>
#include <stdlib.h>
...
sub generate_tracker_data {
    ...
    if ( defined( $ctx->{game_config}->{'tracker'} ) ) {
        ...
        my $trk = $is_cpc ? \@{ $ctx->{c_tracker_cpc_lines} } : \@{ $ctx->{c_banked_data_128_lines} };
        my $trk_dir = $is_cpc
            ? "$ctx->{build_dir}/generated"
            : "$ctx->{build_dir}/generated/banked/128";
        ...
        push @{ $ctx->{h_game_data_lines} }, "/////////////////////\n";
        push @{ $ctx->{h_game_data_lines} }, "// Tracker songs\n";
        push @{ $ctx->{h_game_data_lines} }, "/////////////////////\n\n";
        ...
        foreach my $song ( @{ $ctx->{game_config}->{'tracker'}{'songs'} } ) {
            ...
            if ( $ctx->{game_config}->{'tracker'}{'type'} eq 'vortex2' ) {
                ...
                copy( "$ctx->{build_dir}/$song->{'file'}", "$ctx->{build_dir}/generated/banked/128" ) or
                    die "Could not copy $ctx->{build_dir}/$song->{'file'} to $ctx->{build_dir}/generated/banked/128\n";
                my $dest_asm_file = sprintf( "$ctx->{build_dir}/generated/banked/128/vortex2_song_%s.asm", $symbol_name );
                my $bin_basename = basename( "$ctx->{build_dir}/$song->{'file'}" );
        ...
        if ( defined( $ctx->{game_config}->{'tracker'}{'in_game_song'} ) ) {
            push @{ $ctx->{h_game_data_lines} }, sprintf( "#define TRACKER_IN_GAME_SONG\tTRACKER_SONG_%s\n",
                uc( $ctx->{game_config}->{'tracker'}{'in_game_song'} ) );
        }

        # now output the songs table
        push @{ $trk }, "\n// songs table\n";
        push @{ $trk }, sprintf( "uint8_t *all_songs[ %d ] = {\n",
            scalar( @{ $ctx->{game_config}->{'tracker'}{'songs'} } ) );
        foreach my $song ( @{ $ctx->{game_config}->{'tracker'}{'songs'} } ) {
            my $symbol_name = "tracker_song_" . $song->{'name'};
        ...
        if ( defined( $ctx->{game_config}->{'tracker'}{'fxtable'} ) ) {
            ...
            my $asm_file;
            if ( $ctx->{game_config}->{'tracker'}{'fxtable'}{'file'} =~ m/\.asm$/i ) {
                $asm_file = "$ctx->{build_dir}/$ctx->{game_config}->{'tracker'}{'fxtable'}{'file'}";
            } elsif ( $ctx->{game_config}->{'tracker'}{'fxtable'}{'file'} =~ m/\.aks$/i ) {
                $asm_file = main::arkos2_convert_effects_to_asm( "$ctx->{build_dir}/$ctx->{game_config}->{'tracker'}{'fxtable'}{'file'}", 'all_sound_effects' );
            } else {
            ...
            push @{ $ctx->{h_game_data_lines} }, sprintf( "#define TRACKER_SOUNDFX_NUM_EFFECTS %d\n", $effects_count );

            move( $asm_file, $dest_asm_file ) or
                die "Could not rename $asm_file to $dest_asm_file\n";
        }
        if ( defined( $ctx->{game_config}->{'tracker'}{'fx_channel'} ) ) {
            push @{ $ctx->{h_game_data_lines} }, sprintf( "#define TRACKER_SOUNDFX_CHANNEL %d\n",
                $ctx->{game_config}->{'tracker'}{'fx_channel'} );
        }
        if ( defined( $ctx->{game_config}->{'tracker'}{'fx_volume'} ) ) {
            push @{ $ctx->{h_game_data_lines} }, sprintf( "#define TRACKER_SOUNDFX_VOLUME %d\n",
                $ctx->{game_config}->{'tracker'}{'fx_volume'} );
        }
